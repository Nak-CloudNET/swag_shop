...
function loadItems() {
	if (localStorage.getItem('sloitems')) {
		...
		$.each(sloitems, function () {
			...
			if(!item_discount) {
				if(item_price == 0){
					item_price = item.group_price.price ;
				}
				if(iprice == 0) {
					iprice = item.group_price.price ;
				}
			}
			...
			*/
			...
			if (item_promotion == 1){
				...
				tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + iprice + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right spricesprice" id="sprice_' + row_no + '">' + formatMoney(iprice) + '</span></td>';
			}
