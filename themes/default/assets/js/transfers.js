 ...
 * Misc Actions
 ...
 function loadItems() {
    ...
    if (localStorage.getItem('toitems')) {
        ...
        an = 1;
        product_tax = 0;
		var t_net_unit_cost = 0;
		var t_subtotal = 0;
		
		if(gp){
			t_net_unit_cost = gp["transfers-net_unit_cost"];
			t_subtotal		= gp["transfers-subtotal"];
		}
        $("#toTable tbody").empty();
        ...
        $.each(toitems, function () {
            ...
            if(owner || admin || t_net_unit_cost) {
				...
				tr_html += '<input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + formatDecimal(item_cost) + '"><input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '"></td>';
			}
			...
			if(owner || admin || t_subtotal) {
				...
				tr_html += '<input type="hidden" class="ssubtotal" id="subtotal_' + row_no + '" ' + formatMoney(((parseFloat(item_cost) - item_discount + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '>';
			}
		...
		if(owner || admin){
			...
			if(t_net_unit_cost == 1 || t_subtotal == 1){
					if(t_net_unit_cost == 1 && t_subtotal == 1){
						var col = 2;
					}
					else if(t_subtotal == 1){					
						var col = 1;
		...
		if(owner || admin){
			...
			if(t_net_unit_cost == 1 || t_subtotal == 1){
				 if(t_net_unit_cost == 1 && t_subtotal == 1){
					tfoot += '<th class="text-right">'+formatMoney(total)+'</th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
				$('#toTable tfoot').html(tfoot);
				 } else if(t_net_unit_cost == 1){ tfoot += '<th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; 			filter:alpha(opacity=50);"></i></th></tr>';
					$('#toTable tfoot').html(tfoot); } else { tfoot += '<th class="text-right">'+formatMoney(total)+'</th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
				$('#toTable tfoot').html(tfoot); }
			}else{
