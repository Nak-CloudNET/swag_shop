...
$(document).on('blur', '#suspendroom', function () {
	localStorage.setItem('suspendroom', $(this).val());
	$('#suspendroom').val($(this).val());
});

if (suspendroom = localStorage.getItem('suspendroom')) {
	$('#suspendroom').val(suspendroom);
}
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		$.each(positems, function (i, e) {
			...
			var item_note = '';
			var product_id = item.row.id, item_type = item.row.type, picture = item.row.image, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_note = item.row.note, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
			...
			if(pos_settings.show_product_code == 0) {
				tr_html += '<td class="edit" style="cursor:pointer;"><ul class="enlarges"><li><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="product_note[]" type="hidden" class="rnote" value="' + pn + '"><span class="sname" id="name_' + row_no + '">' + item_name +(sel_opt != '' ? ' ('+sel_opt+')' : '')+ (pn != '' ? ' (<span id="get_not">'+pn+'</span>)' : '')  +'</span>';
				if(pos_settings.show_item_img != 0) {
					tr_html += '<span class="showimg"><a href="'+site.base_url+'assets/uploads/ '+picture+'" data-toggle="lightbox"><img src="'+site.base_url+'assets/uploads/'+picture+'" alt="' +item_name+ '" style="width:200px;" class="img-thumbnail" /></a></span>';
				}
			}else{
