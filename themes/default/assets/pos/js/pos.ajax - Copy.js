...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var formattedDate = "" + d.getDate() + "/" + (d.getMonth()+1) + "/" + d.getFullYear() + " " + h + ":" + m + " " + amPm;

			$("#order_span").empty(); $("#bill_span").empty();
			//var pos_head1 = '<span style="text-align:center;"><h3>'+site.settings.site_name+'</h3><h4>'
			var pos_head2 = '</h4><h5>Customer: '+$('#select2-chosen-1').text()+'<br>In: '+formattedDate+'<br/></h5></span>';
			$("#order_span").prepend('<h5 class="center">Order</h5> '+pos_head2);
			$("#bill_span").prepend('<h5 class="center">Bill</h5>'+pos_head2);
		...
		$.each(positems, function (i, e) {
			...
			if(pos_settings.pos_layout == 3 && pos_settings.show_item_img != 0) {
	...
	if(susp_id > 0){
		...
		if(java_applet == 1) {
			...
			bill_totals += '<tr><td>Total Amount KH</td><td style="text-align:right;">៛ '+bill_amount_kh+'</td></tr>';
