...
class Pos extends MY_Controller
{
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
				...
				".$this->db->dbprefix('sales').".reference_no, biller, customer, grand_total, paid, (grand_total - paid) AS balance, payment_status, companies.email as cemail")
                ->from('sales')
    ...
    function index($sid = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
					...
					//$this->erp->print_arrays($products);exit;
            ...
            $grand_total = $this->erp->formatDecimal($this->erp->formatDecimal($g_total_txt1) + $total_tax + $this->erp->formatDecimal($shipping) - $order_discount);
			...
			//print_r($this->input->post('amount'));exit;
            ...
            $data = array('date' => $date,
                ...
                'total' => $this->erp->formatDecimal($g_total_txt1),
				...
				if(isset($p_cur) && empty($_POST['amount'][0])){
                  for ($j = 0; $j < $p_cur; $j++) {
                    $payment[] = array(
						...
						'date' => $date,
						'reference_no' => $this->site->getReference('sp'),
						'amount' => $this->erp->formatDecimal($_POST['other_cur_paid'] / $cur_rate->rate),
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
			...
			//$this->erp->print_arrays($this->data['room']);
	...
	function complete_kitchen($id = NULL){
		$qty = $this->pos_model->getQtyByID($id);
		$total = $qty->quantity * $qty->unit_price;
		$data = array(
			...
			'warehouse_id' => $qty->warehouse,
			'unit_price'	=> $qty->unit_price,
			'quantity' => $qty->quantity,
			'grand_total' => $total
		);
		//$this->erp->print_arrays($data);
		if($this->pos_model->kitchen_complete($data)){
