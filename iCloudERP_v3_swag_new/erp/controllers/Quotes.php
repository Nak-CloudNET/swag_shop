...
class Quotes extends MY_Controller
{
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("quotes.id, quotes.date, quotes.reference_no, quotes.biller, quotes.customer, quotes.grand_total, payments.amount AS deposit_amount, (erp_quotes.grand_total - COALESCE(erp_payments.amount, 0)) AS balance , quotes.status")
                ->from('quotes')
