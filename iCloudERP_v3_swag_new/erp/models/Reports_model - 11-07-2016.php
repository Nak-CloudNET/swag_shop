...
ORDER BY
    ...
    {
		 $this->db->select("SUM( COALESCE( ABS({$this->db->dbprefix('return_sales')}.grand_total), 0 ) ) AS paid, SUM( ( COALESCE( quantity, 0 ) ) ) AS quantity, SUM(( COALESCE( {$this->db->dbprefix('return_sales')}.order_discount, 0 ) ) ) AS order_discount", FALSE)
						->join('return_items', 'return_items.return_id=return_sales.id')
                        ->join('sales', 'sales.return_id=return_sales.id')
						//->like('quantity', '-')
