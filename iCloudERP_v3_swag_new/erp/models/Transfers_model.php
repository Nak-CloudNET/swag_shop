...
class Transfers_model extends CI_Model
{
    ...
    public function updateTransfer($id, $data = array(), $items = array())
    {
		...
		if ($this->db->update('transfers', $data, array('id' => $id))) {           
			foreach ($items as $item) {
				$item['transfer_id'] = $id;
				//$this->db->update('transfer_items', $item, array('transfer_id' => $id));
				$option_id = $item['option_id'];
				if($option_id){
					$option = $this->transfers_model->getProductOptionByID($option_id);
					$item['quantity_balance'] = $item['quantity'] * $option->qty_unit;
				...
				if ($status == 'completed') { 		
					if($ostatus != 'completed'){
						...
						$item['quantity'] = $item['quantity'] * (-1) ;
						$item['quantity_balance'] = $item['quantity_balance'] * (-1) ;
						$this->db->where(array('transfer_id'=> $id, 'warehouse_id' => $data['from_warehouse_id'], 'product_id' => $item['product_id']));
						$this->db->update('purchase_items', $item);
						...
						$item['quantity'] = $item['quantity'] * (-1) ;
						$item['quantity_balance'] = $item['quantity_balance']  * (-1);
						$this->db->where(array('transfer_id'=> $id, 'warehouse_id' => $data['to_warehouse_id'], 'product_id' => $item['product_id']));
						$this->db->update('purchase_items', $item);
					}
				}else{
