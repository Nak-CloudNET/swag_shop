...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var pos_head2 = '</h4><h5>Room | Table: '+ $(".sus_name").text() +'<br/>Username: '+u_username+'<br/>Customer: '+$('#select2-chosen-1').text()+'<br>In: '+formattedDate+'<br/></h5>';
		...
		$.each(positems, function (i, e) {
			...
			if(layouts != 3) {
				if(pos_settings.show_product_code == 0) {
					...
					if(pos_settings.show_item_img != 0) {
						tr_html += '<span class="showimg" style="width:350px;"><table class="table table-bordered"><tr><th>Image</th>'+(item_type == 'combo' ? '<th>Description</th>' : '')+(item_type == 'combo' ? '<th>Combo Items</th>' : '')+'</tr><tr><td><a href="'+site.base_url+'assets/uploads/'+picture+'" data-toggle="lightbox"><img src="'+site.base_url+'assets/uploads/'+picture+'" alt="' +item_name+ '" style="width:200px;" class="img-thumbnail" /></a></td>'+(item_type == 'combo' ? '<td>'+item_details+'</td>' : '')+(item_type == 'combo' ? '<td><table>'+item.combo_items[0]['name']+'</table></td>' : '')+'</tr></table></span>';
					}
				}else{
            ...
            if (item_promotion == 1){
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item.row.real_unit_price - item_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '</span><input type="hidden" name="grand_total[]" value="' + (((parseFloat(item.row.real_unit_price - item_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '"></td>';
			}
            
			tr_html += '<td class="text-center posdel" style="cursor:pointer;"><i class="fa fa-times tip pointer posdel" id="' + row_no + '" title="Remove" style="cursor:pointer;"></i></td>';
			...
			if (item_promotion == 1){
				...
				total += formatDecimal(((parseFloat(item.row.real_unit_price - item_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
			}
			...
			*/
			if(java_applet == 1) {
				...
				bprTr += '<td style="text-align:right">'+ formatMoney(((parseFloat(item.row.real_unit_price - item_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) +'</td>';
				bprTr += '</tr>';
	...
	if(susp_id > 0){
		...
		if(java_applet == 1) {
			...
			bill_totals += '<td colspan="2" style="text-align:right;font-weight:bold;">'+lang_total+'</td><td style="text-align:right;font-weight:bold;">'+formatMoney(total)+'</td></tr>';
			bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;">'+lang_items+'</td><td style="text-align:right;font-weight:bold;">'+(an - 1) + ' (' + (parseFloat(count) - 1) + ')</td></tr>';
			if(order_discount > 0) {
				bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;">'+lang_discount+'</td><td style="text-align:right;font-weight:bold;">'+ dis_per +'</td></tr>';
			}
			if (site.settings.tax2 != 0 && invoice_tax != 0) {
				bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;">'+lang_tax2+'</td><td style="text-align:right;font-weight:bold;">'+formatMoney(invoice_tax)+'</td></tr>';
			}
			
			var bill_amount_kh = 0;
			bill_amount_kh = gtotal * kh_rate;

			bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;">'+lang_total_payable+'</td><td style="text-align:right;font-weight:bold;">'+formatMoney(gtotal)+'</td></tr>';
			bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;" >Total Amount KH</td><td style="text-align:right;font-weight:bold;">'+formatSA(parseFloat(bill_amount_kh).toFixed(0))+' ៛</td></tr></tfoot>';
