...
class Pos_model extends CI_Model
{
	...
	*/
    ...
    public function closeRegister($rid, $user_id, $data)
    {
		...
		$this->db->query("UPDATE erp_payments SET register_status = '1' WHERE created_by = '".$user_id."' AND type='received'");
    ...
    */
    ...
    public function getRegisterCashSales($date, $user_id = NULL)
    {
        ...
        $this->db->select('sales.other_cur_paid_rate AS khrate,SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('payments.type', 'received')->where('payments.register_status', '0')->where('paid_by', 'cash');
        $this->db->where('payments.created_by', $user_id);
