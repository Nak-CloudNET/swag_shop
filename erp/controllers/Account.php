...
function getBillReciept($pdf = NULL, $xls = NULL)
{
    ...
    public function getDeposits(){
    	...
    	<ul class="dropdown-menu pull-right" role="menu">
    		...
    		$this->load->library('datatables');
    		$this->datatables
    		->select("deposits.id as dep_id, companies.id AS id , date,companies.name,
			 deposits.amount, deposits.paid_by, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as created_by", false)
    		->from("deposits")
	...
	function ar_by_customer()
	{
		...
		if($this->form_validation->run() == true){
			if($this->input->post('start_date')){
				...
				$start_date = $this->erp->fld(date('d/m/Y 00:m:i'));
			}
			
			if($this->input->post('end_date')){
				$end_date = $this->erp->fld($this->input->post('end_date'));
			}else{
				$end_date = $this->erp->fld(date('d/m/Y 23:m:i'));
			}
			...
			$this->data['start_date'] = $this->erp->hrld(date('Y-m-d 00:m:i'));
			$this->data['end_date'] = $this->erp->hrld(date('Y-m-d 23:m:i'));
			$start_date = $this->erp->fld(date('d/m/Y 00:m:i'));
			$end_date =  $this->erp->fld(date('d/m/Y 23:m:i'));
			$balance = 'all';
			$customer = '';
	...
	function ap_by_supplier()
	{
		...
		foreach($suppliers as $sup){
			...
			foreach($supplierDatas as $suppData){
				$supplierPayment = $this->accounts_model->ap_by_supplier($sup->start_date, $sup->end_date, $sup->supplier_id, $sup->balance, 'payment', $suppData->id);
				$k = 0;
				foreach($supplierDatas as $sppD) {
					$supplierDatas[$k]->payments = $this->accounts_model->getPaymentByPurchaseID($sppD->id);
					$k++;
				}
				$my_data[$i] = array(
