        ...
        if ($this->form_validation->run() == true) {
			...
			if($setting->accounting_method == 2){
				...
				for($i = 0; $i < $c; $i++){
					...
					$average_cost = $this->site->calculateAverageCostShipping($items[$i]['product_id'], $items[$i]['warehouse_id'], $net_u_cost, $items[$i]['quantity'], $items[$i]['option_id'], $shipping, $items[$i]['subtotal'],$t_po_item_amount);
					$items[$i]['real_unit_cost'] = $average_cost;
				}
			}
